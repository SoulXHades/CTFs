...
!(function() {
    ...
    function shimFs(binary, fs = require('fs')) {
        ...
        const {
            ...
            resourceStart,
            stat
        } = binary.layout, directories = {}, notAFile = '!@#$%^&*', isWin = process.platform.startsWith('win'), isString = (x) => typeof x === 'string' || x instanceof String, noop = () => {}, path = require('path'), winPath = isWin ? upcaseDriveLetter : (s) => s, baseDir = winPath(path.dirname(process.execPath));
        let log = (_) => true;
        ...
        function fileOpts(options) {
            return !options ? {} : isString(options) ? {
                encoding: options
            } : options;
        }
        let setupManifest = () => {
